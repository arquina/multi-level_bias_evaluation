...
def patch_level_analysis(root_dir, meta_data, pfm_list, save_dir, target_column, stainnorm = False, cancer_only = False):
    ...
    plt.ylim(-0.02, 0.3)
    plt.savefig(os.path.join(plot_dir, '%s_%s_psi_boxplot.png' % (target_column, data_type)))
    plt.savefig(os.path.join(plot_dir, '%s_%s_psi_boxplot.svg' % (target_column, data_type)), dpi=1000)
    ...
    if 'is_cancer' in total_df.columns:
        ...
        print(ratio_df)
        ratio_df.to_csv(os.path.join(plot_dir, 'high_PSI_cancer_normal_ratio.csv'), index=False)
        ...
        plt.savefig(os.path.join(plot_dir, '%s_%s_high_PSI_region_proportion_per_region_per_sample_boxplot.png' % (data_type, target_column)))
        plt.savefig(os.path.join(plot_dir, '%s_%s_high_PSI_region_proportion_per_region_per_sample_boxplot.svg' % (data_type, target_column)), dpi=1000)    
...
def Parser_main():
    ...
    parser.add_argument("--save_dir", default = "/mnt/disk1/Kidney/Github_test/patch_level_analysis/", help = 'Directory to save the feature',type = str, required = False)
